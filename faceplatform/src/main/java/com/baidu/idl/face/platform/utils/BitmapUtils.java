...
//            bitmap.setPixels(argbByte, 0, roundRect.width(), 0, 0, roundRect.width(), roundRect.height());
    ...
    public static File compressToFile(Bitmap bitmap, String path) {
        return compressToFile(bitmap, DEFAULT_JPEG_QUALITY, path);
    }

    public static File compressToFile(Bitmap bitmap, int quality, String path) {
        File file = compressToFile(bitmap, CompressFormat.JPEG, quality, path);
        return file;
    }

    public static File compressToFile(Bitmap bitmap, CompressFormat format, int quality, String path) {
        ...
        try {
            ...
            try {
                ...
                Log.e("name",ex.getMessage()+"2");
                ex.printStackTrace();
            }
        } catch (Exception ex) {
            Log.e("name",ex.getMessage()+"3");
            try {
                if (f != null && f.exists() && f.isFile()) {
                    f.delete();
                }
            } catch (Exception e) {
                Log.e("name",e.getMessage()+"4");
                e.printStackTrace();
            }
        } finally {
